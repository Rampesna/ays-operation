...
class DeviceController extends Controller
{
    ...
    {
        return response()->json(Device::with(['group', 'status', 'employee'])->find($request->device_id), 200);
    }

    public function create(Request $request)
    {
        $device = (new DeviceService(new Device))->store($request);
        return response()->json(Device::with(['group', 'status', 'employee'])->find($device->id), 200);
    }
